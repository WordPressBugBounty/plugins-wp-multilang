...
class WPM_Bulk_Translate {
	...
	protected $screens_array = array( 'edit-post' );
	...
	public function __construct(){
		...
		add_action( 'wpm_admin_field_import_translations', array( $this, 'render_import_translations' ) );
		// add_action( 'admin_post_wpm_import_translations', array( $this, 'import_translations' ) );
		add_action( 'admin_init', array( $this, 'import_translations' ) );
	...
	public function load_scripts(){
		...
		if ( is_object( $this->current_screen ) && isset( $this->current_screen->base ) && in_array( $this->current_screen->base, array( 'edit', 'upload' ) ) ) {

			$suffix = defined ( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

			wp_register_script( 'wpm-bulk-translate', wpm_asset_path( 'scripts/wpm-bulk-translate' . $suffix . '.js' ), array( 'jquery' ), WPM_VERSION, true );		

			$translator_params = array(     
				'ajax_url'                     			=> admin_url( 'admin-ajax.php' ),           
				'wpm_bulk_translate_security_nonce'  	=> wp_create_nonce('wpm_bulk_translate_security_nonce'),
			);
							
			wp_localize_script( 'wpm-bulk-translate', 'wpm_bulk_translate_params', $translator_params );	

			wp_enqueue_script( 'wpm-bulk-translate' );

			wp_enqueue_style( 'wpm-bulk-translate-style', wpm_asset_path( 'styles/admin/wpm-bulk-translate' . $suffix . '.css' ), array(), WPM_VERSION );
	...
	public function init( $current_screen ){
		...
		add_filter( "bulk_actions-{$current_screen->id}", array( $this, 'add_action' ) );
	...
	public function handle_bulk_action( $sendback, $action ) {
		...
		if ( ! is_wp_error( $query_args ) ) {
			$error 	=	$this->perform_bulk_action( $query_args['item_ids'], $query_args['wpm_bt_file_format'] );
	...
	public function display_form() {
		...
		if ( is_object( $this->current_screen ) && ! empty( $this->current_screen->id ) && in_array( $this->current_screen->id, $this->screens_array ) ) {

			require_once __DIR__ . '/views/bulk-translate/template-bulk-translate.php';
	...
	public function parse_bulk_request( $request ) {
		...
		if ( empty( $args['item_ids'] ) ) {
			return new \WP_Error( 'wpm_no_items_selected', esc_html__( 'No item has been selected. Please make sure to select at least one item to be translated.', 'wp-multilang' ) );
		}

		$args['wpm_bt_file_format'] = sanitize_key( $request['wpm_bt_file_format'] );
	...
	private function add_settings_error( \WP_Error $error ) {
		...
		set_transient( 'wpm_bulk_translate_errors_' . get_current_user_id(), get_settings_errors( 'wpm_bulk_errors' ) );
	...
	public function perform_bulk_action( $post_ids, $file_type ){
		...
		if ( $default_lang == $current_lang ) {
			return new \WP_Error( 'invalid-target-languages', __( 'Error: Source and destination languages cannot be same.', 'wp-multilang' ) );
		}
		...
		if ( ! empty( $posts ) && is_array( $posts ) ) {
			...
			// foreach ($posts as $post) {
				...
				switch( $version ) {
					...
					case '20':
						...
						$xliff_content 	=	$this->generate_xliff_20( $posts, $languages, $default_lang, $current_lang );

					break;

					case '21':

						$xliff_content 	=	$this->generate_xliff_21( $posts, $languages, $default_lang, $current_lang );

					break;

					default: // 1.2

						$xliff_content 	=	$this->generate_xliff_12( $posts, $languages, $default_lang, $current_lang );
			...
			$file_name 	= $source_lang.'_'.$target_lang.'_'.date('Y-m-d_H-i-s').'.xliff';
	 ...
	 * @since 	2.4.17
	 * */
	public function format_xliff_data( $posts, $languages, $default_lang, $current_lang ) {
		...
		foreach ( $posts as $key => $post ) {
			...
			$data[$key]['source_lang'] 	=	str_replace( '_', '-', $languages[$default_lang]['locale'] );
			$data[$key]['target_lang'] 	=	str_replace( '_', '-', $languages[$current_lang]['locale'] );
			$data[$key]['original'] 	=	'WP Multilang |'.WPM_VERSION.'|'.get_home_url();

			$source_post_title 			=	wpm_translate_string( $post->post_title, $default_lang );
			$target_post_title 			=	'';
			if ( strpos( $post->post_title, '[:]' ) !== false ) {
				$target_post_title 		=	wpm_translate_string( $post->post_title, $current_lang );
			...
			$source_content 			=	wpm_translate_string( $post->post_content, $default_lang );
			$target_content 			=	'';
			if ( strpos( $post->post_content, '[:]' ) !== false ) {
				$target_content 		=	wpm_translate_string( $post->post_content, $current_lang );
	 ...
	 * @since 	2.4.17
	 * */
	public function generate_xliff_12( $posts, $languages, $default_lang, $current_lang ) { 

		$original 			=	get_home_url();
		$product_version 	=	WPM_VERSION;
		$unit_id 			=	1;

		$formatted_data 	=	$this->format_xliff_data( $posts, $languages, $default_lang, $current_lang );
	 ...
	 * @since 	2.4.17
	 * */
	public function generate_xliff_20( $posts, $languages, $default_lang, $current_lang ) {
	 ...
	 * @since 	2.4.17
	 * */
	public function generate_xliff_21( $posts, $languages, $default_lang, $current_lang ) {
	...
	public function import_translations() {
		...
		if ( isset( $_POST['wpm_import_xliff_btn'] ) ) {
			...
			$btn_text 		=	sanitize_text_field( wp_unslash( $_POST['wpm_import_xliff_btn'] ) );
			if ( $btn_text == 'Import xliff data' ) {

				if ( ! empty( $_FILES['wpm_import_xliff_file'] ) && ! empty( $_FILES['wpm_import_xliff_file']['tmp_name'] ) ) {

					$file_name 		=	sanitize_text_field( $_FILES['wpm_import_xliff_file']['name'] );
					$extension 		=	pathinfo( $file_name, PATHINFO_EXTENSION );
					$filepath 		=	sanitize_text_field( $_FILES['wpm_import_xliff_file']['tmp_name'] );
	...
	public function import_data_from_file( $filepath ){
		...
		if ( false === $file_contents ) {
			return new \WP_Error(
				...
				__( 'Something went wrong during the file import.', 'wp-multilang' )
			);
		}

		libxml_use_internal_errors(true);
		$dom 		=	new \DOMDocument('1.0', 'UTF-8');
		$isxml 		=	$dom->loadHTML( $file_contents, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD );
		libxml_clear_errors();
					...
					foreach ( $file_tags as $file_tag ) {
						...
						if ( is_object( $file_tag ) && ! empty( $file_tag ) ) {
							...
							if ( empty( $target_lang ) ) {
								$target_lang 	=	$file_tag->getAttribute('target-language');
							}

							switch( $version ) {

								case '1.2':
									$post_data 	=	$this->xml_to_array_1_2( $file_tag );
								break;

								case '2.0':
								case '2.1':
									$post_data 	=	$this->xml_to_array_2_1( $file_tag );
								break;
		...
		if ( is_array( $post_data ) && ! empty( $post_data ) && ! empty( $target_lang ) ) {
			...
			if ( array_key_exists( $lang, $languages ) ) {
				foreach ( $post_data as $post_id => $post ) {
					...
					if ( ! empty( $post['post_title'] ) ) {
						...
						$title 			=	wpm_set_new_value( $post_title, $post['post_title'], array(), $lang );
						
					}
					if ( ! empty( $post['post_content'] ) ) {
						$post_content	=	$get_post->post_content;
						$content 		=	wpm_set_new_value( $post_content, $post['post_content'], array(), $lang );
				...
				return new \WP_Error(
					'wpm_import_file_contents_error',
					__( 'Error: You are trying to import a file in a language which doesn\'t exist on your site', 'wp-multilang' )
				);	
			}
			...
			foreach ( $body_tag as $body ) {
				...
				if ( is_object( $group_tag ) && ! empty( $group_tag->length ) ) {
					foreach ( $group_tag as $group ) {
						...
						if ( is_array( $group_attr ) && ! empty( $group_attr['restype'] ) ) {
							if ( $group_attr['restype'] == 'x-post' && ! empty( $group_attr['resname'] ) ) {

								$post_id 		=	intval( $group_attr['resname'] );
								if ( $post_id > 0 ) {

									$trans_unit 	=	$group->getElementsByTagName('trans-unit');
									if ( is_object( $trans_unit ) && ! empty( $trans_unit->length ) ) {

										foreach ( $trans_unit as $trans ) {

											$restype 		=	$trans->getAttribute('restype');
											if ( $restype == 'x-post_title' ) {
												$target 	=	$trans->getElementsByTagName('target');
												if ( is_object( $target ) && ! empty( $target->item(0) ) ) {
													$post_data[$post_id]['post_title'] 	=	sanitize_text_field( $target->item(0)->nodeValue );
												}
											} else if ( $restype == 'x-post_content' ) {
												$target 	=	$trans->getElementsByTagName('target');
												if ( is_object( $target ) && ! empty( $target->item(0) ) ) {
													$post_data[$post_id]['post_content'] 	=	sanitize_text_field( $target->item(0)->nodeValue );
												}
											}	
	...
	public function xml_to_array_2_1( $file_tag ){
		...
		if ( is_object( $group_tag ) && ! empty( $group_tag->length ) ) {
			...
			foreach ( $group_tag as $group ) {
				...
				if ( ! empty( $group_attr['type'] ) && $group_attr['type'] == 'x:post' ) {
					...
					if ( is_object( $units ) && ! empty( $units->length ) ) {
						...
						foreach ( $units as $unit ) {
							...
							if ( is_object( $segments ) && ! empty( $segments ) ) {
								...
								}else if ( $unit_type == 'x:post_content' ) {

									foreach ( $segments as $segment) {

										$target 	=	$segment->getElementsByTagName('target');
										if ( is_object( $target ) && ! empty( $target->item(0) ) ) {
											$post_data[$post_id]['post_content'] 	=	$target->item(0)->nodeValue;
										}
